...
(defvar syslog-mode-map
  (let ((map (make-sparse-keymap)))
    ;; Ctrl bindings
    (define-key map (kbd "C-c /") 'syslog-filter-lines)
    (define-key map (kbd "C-c C-a") 'hide-lines-show-all)
    (define-key map (kbd "C-c C-h") 'hide-lines-matching)
    (define-key map (kbd "C-c C-s") 'hide-lines-not-matching)
    (define-prefix-command 'syslog-highlight-map)
    (define-key map (kbd "C-c h") 'syslog-highlight-map)
    (define-key map (kbd "C-c h r") 'highlight-regexp)
    (define-key map (kbd "C-c h p") 'highlight-phrase)
    (define-key map (kbd "C-c h l") 'highlight-lines-matching-regexp)
    (define-key map (kbd "C-c h u") 'unhighlight-regexp)
    (define-key map (kbd "C-c C-/") 'syslog-filter-dates)
    (define-key map (kbd "C-c C-=") 'syslog-count-matches)
...
highlight those regexps with."
  (interactive (let ((view (cdr (cl-assoc (completing-read "View: " (mapcar 'car syslog-views))
...
\\{syslog-mode-map}"
  ...
  (easy-menu-define nil syslog-mode-map "test"
    `("Syslog"
      ...
      ["Revert buffer" revert-buffer :help "View the function at point" :key "g"]
      ["Show all"  hide-lines-show-all :help "Show all hidden lines/blocks" :key "r"]
...
(defface syslog-hour
  '((t :weight bold  :foreground "tomato"))
  "Face for hours"
  :group 'syslog)
...
(defvar syslog-font-lock-keywords
  '(
    ...
    ("\\<[Nn][Oo][Tt][Ii][Cc][Ee]\\>" 0 'syslog-info append)
    ...
    ;; Dates: Sat May 6 14:13:51 CST 2017
    ("\\(?:^\\|[[:space:]]\\)\\(\\(\\(\\<Sun\\>\\|\\<Mon\\>\\|\\<Tue\\>\\|\\<Wed\\>\\|\\<Thu\\>\\|\\<Fri\\>\\|\\<Sat\\>\\)[[:space:]]+\\)?\\(\\(\\<Jan\\(uary\\)?\\>\\|\\<Feb\\(ruary\\)?\\>\\|\\<Mar\\(ch\\)?\\>\\|\\<Apr\\(il\\)?\\>\\|\\<May\\>\\|\\<Jun\\(e\\)?\\>\\|\\<Jul\\(y\\)?\\>\\|\\<Aug\\(ust\\)?\\>\\|\\<Sep\\(tember\\)?\\>\\|\\<Oct\\(ober\\)?\\>\\|\\<Nov\\(ember\\)?\\>\\|\\<Dec\\(cember\\)?\\>\\)[[:space:]]+\\)?\\([[:digit:]]\\{1,2\\}[[:space:]]+\\)?[[:digit:]]+\\{2\\}:[[:digit:]]\\{2\\}:[[:digit:]]\\{2\\}\\([[:space:]]+\\(\\<CEST\\>\\|\\<UTC\\>\\|\\<CST\\>\\|\\<PDT\\>\\)\\)?\\([[:space:]]+\\([[:digit:]]\\{4\\}\\)\\)?\\)\\(?:$\\|[[:space:]]\\)" 1 'syslog-hour append)

    ;; ("\\(?:^\\|[[:space:]]\\)\\(\\<[Jj]an\\(?:uary\\)?\\>\\|\\<[Ff]eb\\(?:ruay\\)?\\>\\|\\<[Jj]un\\(?:e\\)?\\>\\)\\([[?:$\\|[[:space:]]]]\\)" 1 'my-syslog-hour append)
    ;; ("\\(?:^\\|[[:space:]]\\)\\(\\([[:alpha:]]\\{3\\}[[:space:]]\\)\\{1,2\\}\\([[:digit:]]\\{1,2\\}[[:space:]]\\{1,2\\}\\)?[[:digit:]]\\{2\\}:[[:digit:]]\\{2\\}:[[:digit:]]\\{2\\}\\([[:space:]][[:space:]]\\)?\\([[:space:]][[:digit:]]\\{4\\\}\\)?\\)" 1 'syslog-hour append)
    ;; Hours: 17:36:00
    ("\\(?:^\\|[[:space:]]\\)\\([[:digit:]]\\{2\\}:[[:digit:]]\\{2\\}:[[:digit:]]\\{2\\}\\(.[[:digit:]]\\{3,8\\}\\)?\\([[:space:]]?[PpAa][Mm]\\)?\\)\\(?:$\\|[[:space:]]\\)" 1 'syslog-hour )
    ;; Dates: 2017-12-32
    ("\\(?:^\\|[[:space:]]\\)\\([[:digit:]]\\{2,4\\}[-/][[:digit:]]\\{1,2\\}[-/][[:digit:]]\\{1,2\\}\\)\\(?:$\\|[[:space:]]\\)" 1 'syslog-hour append)
    ;; ("\\(?:^\\|[[:space:]]\\)\\([[:digit:]]\\{6,17\\}\\)\\(?:$\\|[[:space:]]\\)" 1 'syslog-hour append)
